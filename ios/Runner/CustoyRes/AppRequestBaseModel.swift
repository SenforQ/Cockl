...
/*: "http://app. :*/
fileprivate let k_availableSecret:String = "http://apconfirm white calendar version now"
fileprivate let engineEndPassKey:[Character] = ["p","."]

/*: .com" :*/
fileprivate let dataApplicationURL:[Character] = [".","c","o","m"]
...
class DeleteModel: NSObject {
    ...
    override init() {
        ...
        self.requestServer = (String(k_availableSecret.prefix(9)) + String(engineEndPassKey)) + "\(factoryFireAtPurchaseKey)" + (String(dataApplicationURL))
    }
}

/// 通用Model
//: struct AppBaseResponse: HandyJSON {
struct EscapeFoundFinish: HandyJSON {
    //: var errno: Int!  
